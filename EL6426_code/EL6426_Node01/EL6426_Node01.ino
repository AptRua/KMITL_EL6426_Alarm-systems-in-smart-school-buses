...
float calibration_factor =58905; 
#define zero_factor -43285
float offset=0.342;
float get_units_kg();
String data;
float weigh;
...
WidgetLED LED1(V1);WidgetLED LED5(V5);WidgetLED LED9(V9);
WidgetLCD LCD1(V15);
...
void setup() 
{
  ...
  Blynk.begin(auth, ssid, pass); LED1.off(); LED5.off(); LED9.off(); LCD1.clear();Blynk.virtualWrite(V11,0);
...
void alert()
{
  ...
  digitalWrite(LED,LOW);
  LCD1.clear();
  LED1.on(); LED5.off(); LED9.on();Blynk.virtualWrite(V11,data);
  delay(500);
  analogWrite(BUZZER,1000);
  digitalWrite(LED,HIGH);
  delay(500);
  LCD1.print(0,0,"SOMEONE IN CAR!");
  LED1.off(); LED5.off(); LED9.off();
...
void read_weigh()
{
  ...
  if(weigh>1.00){
    ...
    Serial.print(data);
  Serial.print(" kg  ");
  Blynk.virtualWrite(V11,data);LED5.on();
  Serial.print("\"OBJECT\"");
  ...
  delay(1000);
    ...
    LED5.off(); Blynk.virtualWrite(V11,0);
    Serial.print("Reading: ");
